...
We support Composite primary keys while still supporting the default devonfw values with Long id.

Below example shows how composite primary keys can be enabled.
For an employee, the companyID and the employeeID together make up a composite primary key as shown below.
...
@Entity
@Table(name = "employee")
public class EmployeeEntity implements Employee, PersistenceEntity<CompositeEmployeeKey> {
...
EmployeeEntity should implement PersistenceEntity<ID>.java in the EmployeeEntity and pass the composite primary key object which is CompositeEmployeeKey in this case as the parameter ID. 

The modificationCounter will be increased whenever the object gets modified and persisted, EmployeeEntity has to implement the below methods from the PersistenceEntity<ID>. The sample implementation of the modification counter has been provided above.
