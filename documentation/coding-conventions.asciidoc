...
| «layer» | The name of the technical layer (See link:architecture.asciidoc[technical architecture]) which is one of the predefined layers as described blow in xref:#layers[layers] section. | `logic`
...
With trends such as microservices, lean, and agile we decided for a more modern packaging and layering that fits better to recent market trends.
...
The following table describes our initial approach for packaging and layering.
It is still supported and fine to use.
However, when you start a new project, you should consider using the new modern structure as described above.

The generic name of the service layer as initially suggested in `devon4j`.
...
(`dataaccess`, `logic`, `service`, `batch`, `gui`, `client`) or `common` for code not assigned to a technical layer (datatypes, cross-cutting concerns).
