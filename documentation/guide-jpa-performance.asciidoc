...
== Advanced tuning
Under the hood everything is still link:guide-jdbc.asciidoc#jdbc[JDBC].
Therefore `ResultSet` is used to get result data from the DB.
The `JDBC fetch size` parameter configures how many rows are read by `ResultSet` from the DB per page.
A typical default is `10` but it can boost your performance to increase this value for specific queries:
```
// 100 is just an example, always meassure to get it right
