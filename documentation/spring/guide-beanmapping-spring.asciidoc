...
To recreate the mapping from `persistentEntity` (`ApplicationPersistenceEntity` or `GenericEntity`) to `AbstractEto` like with Dozer above, add the following to your _BeansOrikaConfig.java_:
[source,java]
----
  @Override
  @Bean
  public MapperFacade getOrika() {
    ...
    // your custom converters, mappers, factories...
    
    this.mapperFactory.classMap(GenericEntity.class, AbstractEto.class).favorExtension(true) //
        .customize(new CustomMapperEto()) // Devon Mapper circumventing the by default unaccessible persistentEntity
        .byDefault().register(); //
