...
The downside to this is that database portability is lost because there is no abstraction layer and the query may not work with another database because it may use a different syntax.
