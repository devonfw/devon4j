...
Developers that are new to `devon4j` often ask which type should go into `api` and which into `core`. Therefore, you can have a look at our link:coding-conventions.asciidoc#architecture-mapping[architecture-mapping] and you will find the module (`api` or `core`) in the brackets for each type.
