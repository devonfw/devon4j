...
IdRef can be used to reference the other entities in TOs in order to make them type-safe and semantically more expressive.
...
As you can easily see, you can now see that the result and the parameters are foreign-keys.
Further, the generic also shows us the entity (business-object) that is referenced.
We can read the javadoc of these entities and understand the context.
...
To boost the performance when running multiple tests, spring is smart enough to avoid creating the same spring-context multiple times.
Instead it stores or caches them so that if a test-case is executed with a specific spring-configuration that has already been setup before,
the same spring-context can be reused instead of creating it again from scratch.
However, your tests may have multiple spring configurations leading to multiple spring-context.
