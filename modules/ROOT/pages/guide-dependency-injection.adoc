...
To make this work there must be exactly one bean in the container (e.g. xref:spring.adoc[spring] or xref:quarkus.adoc[quarkus]) that is an instance of `MyOtherComponent`.
...
Please note that with xref:quarkus.adoc[quarkus] you will get them as transitive dependencies out of the box.
