...
For transaction handling we xref:guide-aop.adoc[AOP] to add transaction control via annotations as aspect.
This is done by annotating your code with the `@Transactional` annotation.
You can either annotate your xref:guide-dependency-injection.adoc#key-principles[container bean] at class level to make all methods transactional or your can annotate individual methods to make them transactional:
...
This may be done in a xref:guide-service-layer.adoc[service operation] by catching a `ConstraintViolationException` (`org.hibernate.exception.ConstraintViolationException` for hibernate).
As a generic approach you can solve this via xref:guide-rest.adoc#rest-exception-handling[REST execption handling].

== Batches
Transaction control for batches is a lot more complicated and is described in the xref:guide-batch-layer.adoc[batch layer].