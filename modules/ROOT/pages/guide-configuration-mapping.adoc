...
If you are using `spring-boot` as suggested by `devon4j` your application can be configured by `application.properties` file as described in xref:guide-configuration.adoc[configuration].
...
Further to access this configuration, we can use standard xref:guide-dependency-injection.adoc[dependency-injection]:
```java
@Inject
