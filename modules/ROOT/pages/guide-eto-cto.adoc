...
== ETO
For each xref:guide-jpa.adoc#entity[persistent entity] `«BusinessObject»Entity` we create or generate a corresponding _entity transfer object_ (ETO) named `«BusinessObject»Eto`. It has the same properties except for relations.
...
xref:guide-beanmapping.adoc[bean-mapper] can properly map all properties between entity and ETO.
