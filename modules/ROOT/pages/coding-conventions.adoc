...
| «layer» | The name of the technical layer (See link:architecture.adoc[technical architecture]). Details are described for the link:guide-structure-modern.asciidoc#layers[modern project structure] and for the link:guide-structure-classic.asciidoc#layers[classic project structure]. | `logic`
...
This is not about data objects like an link:guide-jpa.asciidoc#entity[entity] or link:guide-transferobject.adoc[transfer-object] that are stateful by design.
Instead this applies to all classes annotated with `@Named`, `@ApplicationScoped`, `@Stateless`, etc. and all their super-classes.
These classes especially include your link:guide-repository.adoc[repositories], link:guide-usecase.adoc[use-cases], and link:guide-rest.asciidoc#jax-rs[REST services].
