...
public class HttpRestClientTest extends ComponentTest {
  ...
  public void testGetWithUrlParam() {
    ...
    // given
    SyncServiceClient<MyExampleRestService> serviceClient = this.serviceClientFactory.createSync(
  ...
  *//**
      ...
      * {@link AsyncServiceClient#call(Object)} testing {@link CompletableFuture} using a HTTP GET operation.
      *
   ...
   *//**
      ...
      */
  /*
   * @Test public void testPostWithTo() {
   *
   * // given AsyncServiceClient<MyExampleRestService> serviceClient = this.serviceClientFactory.createAsync(
   * MyExampleRestService.class, new
   ...
   *
   * // when TestResultHandler<MyExampleTo> resultHandler = new TestResultHandler<>(serviceClient);
   * serviceClient.setErrorHandler(resultHandler.getErrorHandler());
   * serviceClient.call(serviceClient.get().saveExample(data), resultHandler); // then MyExampleTo response =
   * resultHandler.getResponseOrWait(); assertThat(response.getName()).isEqualTo("John Doe-saved");
   * assertThat(response.getBirthday()).isEqualTo(LocalDate.of(2000, 1, 1)); }
   *
   *//**
      * Test of {@link ServiceClientFactory#createAsync(Class, java.util.Map)} with
      * {@link AsyncServiceClient#call(Object)} testing {@link CompletableFuture} using a HTTP POST operation.
      *
      * @throws Exception on error
      */
  /*
   ...
   * new MyExampleTo(); data.setName("John Doe"); data.setBirthday(LocalDate.of(1999, 12, 31));
   *
   * // when CompletableFuture<MyExampleTo> future = serviceClient.call(serviceClient.get().saveExample(data)); // then
   * MyExampleTo response; response = future.get(); assertThat(response.getName()).isEqualTo("John Doe-saved");
   * assertThat(response.getBirthday()).isEqualTo(LocalDate.of(2000, 1, 1)); }
   *
   *//**
      ...
      */
  /*
   * @Test public void testBusinessError() {
   *
   * // given AsyncServiceClient<MyExampleRestService> serviceClient = this.serviceClientFactory.createAsync(
   * MyExampleRestService.class, new
   ...
   * serviceClient.setErrorHandler(resultHandler.getErrorHandler()); serviceClient.callVoid(() -> {
   * serviceClient.get().businessError(); }, resultHandler); // then Throwable error = resultHandler.getErrorOrWait();
   ...
   *//**
      ...
      */
  /*
   * @Test public void testTechnicalError() {
   *
   * // given AsyncServiceClient<MyExampleRestService> serviceClient = this.serviceClientFactory.createAsync(
   * MyExampleRestService.class, new
   * ServiceClientConfigBuilder().authBasic().userLogin("admin").userPassword("admin").buildMap()); // when
   * TestResultHandler<Void> resultHandler = new TestResultHandler<>(serviceClient);
   * serviceClient.setErrorHandler(resultHandler.getErrorHandler()); serviceClient.callVoid(() -> {
   * serviceClient.get().technicalError(); }, resultHandler); // then Throwable error = resultHandler.getErrorOrWait();
   * assertThat(error).isInstanceOf(ServiceInvocationFailedException.class); ServiceInvocationFailedException e =
   * (ServiceInvocationFailedException) error; assertThat(e.getNlsMessage().getMessage()).matches(
   ...
   * MyExampleRestService.class, new
   * ServiceClientConfigBuilder().authBasic().userLogin("admin").userPassword("admin").buildMap()); // when
   * CompletableFuture<Void> future = serviceClient.callVoid(() -> { serviceClient.get().businessError(); }); // then
   * try { future.get(); failBecauseExceptionWasNotThrown(ServiceInvocationFailedException.class); } catch (Throwable
   * executionEx) { Throwable error = executionEx.getCause();
   * assertThat(error).isInstanceOf(ServiceInvocationFailedException.class); ServiceInvocationFailedException e =
   * (ServiceInvocationFailedException) error; assertThat(e.getNlsMessage().getMessage()).matches(
   * "While invoking the service com\\.devonfw\\.test\\.app\\.myexample\\.service\\.api\\.rest\\.MyExampleRestService#businessError\\[http://localhost:[0-9]+/app/services/rest/my-example/v1/business-error\\] the following error occurred: Test of business error.* Probably the service is temporary unavailable\\. Please try again later\\. If the problem persists contact your system administrator\\."
   * ); assertThat(e.getCode()).isEqualTo(MyBusinessException.CODE); assertThat(e.isForUser()).isTrue();
   ...
   * MyExampleRestService.class, new
   * ServiceClientConfigBuilder().authBasic().userLogin("admin").userPassword("admin").buildMap()); // when
   * CompletableFuture<Void> future = serviceClient.callVoid(() -> { serviceClient.get().technicalError(); }); // then
   * try { future.get(); failBecauseExceptionWasNotThrown(ServiceInvocationFailedException.class); } catch (Throwable
   * executionEx) { Throwable error = executionEx.getCause();
   * assertThat(error).isInstanceOf(ServiceInvocationFailedException.class); ServiceInvocationFailedException e =
   * (ServiceInvocationFailedException) error; assertThat(e.getNlsMessage().getMessage()).matches(
   ...
   *//**
      * Test of {@link ServiceClientFactory#createAsync(Class, java.util.Map)} with
      * {@link AsyncServiceClient#call(Object, java.util.function.Consumer)} testing callback usage to retrieve a
      * primitive {@code boolean} result and conversion to {@link Boolean}.
      */
  /*
   * @Test public void testPrimitiveResult() {
   *
   * // given AsyncServiceClient<MyExampleRestService> serviceClient = this.serviceClientFactory.createAsync(
   * MyExampleRestService.class, new
   * ServiceClientConfigBuilder().authBasic().userLogin("admin").userPassword("admin").buildMap()); // when
   ...
   * serviceClient.setErrorHandler(resultHandler.getErrorHandler());
   * serviceClient.call(serviceClient.get().primitiveResult(), resultHandler); // then
   * assertThat(resultHandler.getResponseOrWait()).isTrue(); }
   *
   *//**
      * Test of {@link ServiceClientFactory#createAsync(Class, java.util.Map)} with
      * {@link AsyncServiceClient#call(Object)} testing {@link CompletableFuture} with a primitive {@code boolean}
      * result and conversion to {@link Boolean}.
      *
      * @throws Exception on error
      *//*
